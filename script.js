...
(function() {
    ...
    let currentUser = null; // নতুন: বর্তমান ব্যবহারকারীকে ট্র্যাক করার জন্য
    let currentView = 'shoppingList'; // 'shoppingList' or 'ledger'
    ...
    let currentLedgerView = 'main'; // 'main' or 'customer'
    ...
    let activeEditingCustomer = null; // MODIFICATION: For customer edit
    ...
    // const backupBtnContainer = document.querySelector('#backupBtn').parentElement;
    ...
    const editItemPriceInput = document.getElementById('itemPrice');
    ...
    // const WELCOME_VIEW_COUNT_KEY = 'welcomeViewCount';
    // const MAX_WELCOME_VIEWS = 3;
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            // প্রোফাইল মেনুতে ব্যবহারকারীর নাম প্রদর্শন
            ...
            console.log(`ব্যবহারকারী লগইন করেছেন: ${user.displayName}`);
    ...
    function signInWithGoogle() {
        ...
        auth.signInWithPopup(provider)
            .then(result => {
                ...
                if (user) {
                    // সফলভাবে সাইন ইন করার পরেই কেবল স্বাগতম বার্তা দেখানো হবে
    ...
    function migrateLedgerData() {
        Object.values(ledgers).forEach(ledger => {
            if (ledger && ledger.transactions && Array.isArray(ledger.transactions)) {
                ...
                console.log("Migration complete for this ledger.");
            }
        });
        saveAllData(); // Save after migration
    }
    ...
    function loadDataFromFirestore() {
        ...
        userDocRef.get().then(doc => {
            if (doc.exists) {
                // ব্যবহারকারীর পুরানো ডেটা পাওয়া গেছে
                const data = doc.data();
                ...
                // এটি একজন নতুন ব্যবহারকারী, তার জন্য ডিফল্ট ডেটা তৈরি করুন
                console.log("নতুন ব্যবহারকারী, ডিফল্ট ডেটা তৈরি করা হচ্ছে।");
                ...
                saveAllData(); // নতুন ব্যবহারকারীর জন্য এই ডিফল্ট ডেটা সেভ করে দিন
            }

            // ডেটা লোড হওয়ার পর UI আপডেট করুন
            migrateLedgerData(); // পুরনো ফরম্যাট থাকলে মাইগ্রেট করুন
            populateListSelector();
            populateLedgerSelector();
            populateReportListFilter();
            loadDarkModePreference(); // Firestore থেকে লোড করা যেতে পারে
            loadFontSizePreference(); // Firestore থেকে লোড করা যেতে পারে
            renderList();
            renderLedger();
    ...
    function saveAllData() {
        if (!currentUser) {
            console.warn("ব্যবহারকারী লগইন করা নেই। ডেটা সেভ করা হচ্ছে না।");
            return;
        }
        ...
        db.collection('userData').doc(currentUser.uid).set(dataToSave)
            .then(() => {
                console.log("ডেটা সফলভাবে ক্লাউডে সেভ হয়েছে!");
            })
            .catch(error => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // loadAllData(); - এই ফাংশনটি এখন আর এখানে কল হবে না
        updateCurrentYear();
        initializeVoiceInput();
        setupAutocomplete();
        setupEventListeners();
        
        // renderList(); - লগইন করার পর ডেটা লোড হলে রেন্ডার হবে
        // renderLedger(); - লগইন করার পর ডেটা লোড হলে রেন্ডার হবে
    ...
    function switchView(view) {
        ...
        // Hide all buttons initially, then show the relevant ones
        shareBtn.style.display = 'none';
        reportBtn.style.display = 'none';
        downloadPdfBtn.style.display = 'none';
        clearBtn.style.display = 'none';
        // importBtn.style.display = 'none';
        // backupBtnContainer.style.display = 'none';

        if (view === 'shoppingList') {
            shoppingListSection.style.display = 'block';
            showShoppingListBtn.classList.add('active');
            
            // Show buttons for shopping list
            shareBtn.style.display = 'inline-flex';
            reportBtn.style.display = 'inline-flex';
            downloadPdfBtn.style.display = 'inline-flex';
            clearBtn.style.display = 'inline-flex';
            // importBtn.style.display = 'inline-flex';
            // backupBtnContainer.style.display = 'inline-flex';
    ...
    function switchList(newId) {
        ...
        // saveAllData(); - No need to save on switch, will save on data change
        currentListId = newId;
        shoppingItems = shoppingLists[currentListId].items;
        // localStorage.setItem('currentListId', currentListId); - No longer needed
        renderList();
        currentListSelector.value = currentListId;
        saveAllData(); // Save the new currentListId
    }
    ...
    async function deleteCurrentList() {
        ...
        if (confirmDelete) {
            ...
            // saveAllData(); - switchList already calls save
    ...
    function switchLedger(newId) {
        ...
        saveAllData(); // Save the new currentLedgerId
    }
    ...
    async function renameLedger() {
        ...
        if (newName && newName.trim() !== '') {
            const trimmedName = newName.trim();
            currentLedger.name = trimmedName;
            populateLedgerSelector();
            saveAllData();
    ...
    async function deleteCurrentLedger() {
        ...
        if (confirmDelete) {
            ...
            // saveAllData(); - switchLedger already saves
    ...
    function saveEditedItem() {
        ...
        const newQuantity = parseInt(editItemQuantityInput.value) || 1;
        const newPrice = parseFloat(editItemPriceInput.value) || 0;
        const newPriority = editItemPriorityInput.value;
        if (newName === '') {
            customAlert('অনুগ্রহ করে পণ্যের নাম লিখুন।', "ইনপুট ত্রুটি");
            return;
        }
        shoppingItems[editingItemIndex].name = newName;
        shoppingItems[editingItemIndex].quantity = newQuantity;
        shoppingItems[editingItemIndex].price = newPrice;
        shoppingItems[editingItemIndex].priority = newPriority;
        renderList(); saveAllData(); closeEditItemModal();
    ...
    function closeTxConfirmModal() {
        ...
        document.body.style.overflow = '';
        showMainLedgerView(); // USER REQUEST: Go back to main ledger view after closing.
    }
    ...
    async function clear() {
        if (currentView === 'shoppingList') {
            if (shoppingItems.length === 0) {
                customAlert("তালিকাটি ইতোমধ্যে খালি।", "তথ্য");
                return;
            }
            ...
            if (!ledgers[currentLedgerId]?.customers?.length) {
                customAlert("খাতাটি ইতোমধ্যে খালি।", "তথ্য");
                return;
            }
    ...
    async function downloadPdf() {
        ...
        try {
            if (currentView === 'shoppingList') {
                ...
                // This button is hidden in ledger view, but we keep the logic just in case.
    ...
    async function downloadCustomerReportPdf(customerId) {
        ...
        try {
            ...
            const addPageNumber = () => {
                // Page number removed as per user request. This function is now empty.
            };

            const checkAndAddPage = (requiredHeight) => {
                if (y + requiredHeight > PAGE_HEIGHT - MARGIN - 10) { // Keep some footer margin
                    addPageNumber();
                    doc.addPage();
                    pageNumber++;
                    y = MARGIN;
            ...
            pdfLedgerContent.innerHTML = ''; // Clear the container
            ...
            const addTableHeader = () => {
                if (checkAndAddPage(tableHeaderHeight)) { // Check if new page was added
                   // If so, y is already at margin, no need to add more space
                }
            ...
            if(checkAndAddPage(footerHeight)) {
                // if footer starts on new page, it might look odd. But it's better than being cut off.
            }
            doc.addImage(footerImg, 'PNG', MARGIN, y, CONTENT_WIDTH, footerHeight);
            y += footerHeight;
            pdfLedgerContent.innerHTML = '';
            
            // 6. Add final page number
            addPageNumber();
    ...
    function toggleDarkMode() {
        ...
        saveAllData(); // Save preference to Firestore
    }
    ...
    function loadFontSizePreference() {
        const userDocRef = db.collection('userData').doc(currentUser.uid);
        userDocRef.get().then(doc => {
            if (doc.exists && doc.data().fontSize) {
                currentFontSize = parseFloat(doc.data().fontSize);
            } else {
    ...
    // --- Backup and Import (Removed as they are redundant now) ---

    // --- Welcome & Monthly Modals ---
    function checkAndShowMonthlyReport() {
        try {
            const today = new Date(); const lastReport = localStorage.getItem(LAST_MONTHLY_REPORT_KEY);
            const currentMonthYear = `${today.getFullYear()}-${today.getMonth()}`;
            if (today.getDate() >= 25 && lastReport !== currentMonthYear) {
                ...
                showMonthlySpendingReport(start, end);
                localStorage.setItem(LAST_MONTHLY_REPORT_KEY, currentMonthYear);
            }
        } catch(e) { console.error("Could not process monthly report logic."); }
    }
    function showMonthlySpendingReport(startDate, endDate) {
        const allItems = Object.values(shoppingLists).flatMap(list => list.items.map(item => ({...item, listName: list.name })));
        const filteredItems = allItems.filter(item => { const itemDate = item.addedDate ? new Date(item.addedDate) : null; return itemDate && itemDate >= startDate && itemDate <= endDate; });
        const summary = calculateReportSummary(filteredItems);

        if (summary.totalItems === 0) return;

        document.getElementById('monthlyReportTotalLists').textContent = toBengaliNumber(String(summary.listIds.size));
        document.getElementById('monthlyReportTotalItems').textContent = toBengaliNumber(String(summary.totalItems));
        document.getElementById('monthlyReportOverallTotal').textContent = formatCurrency(summary.overallTotal);
        document.getElementById('monthlyReportBoughtItemsCount').textContent = toBengaliNumber(String(summary.boughtItemsCount));
        document.getElementById('monthlyReportBoughtItemsTotal').textContent = formatCurrency(summary.boughtItemsTotal);
        document.getElementById('monthlyReportUnboughtItemsCount').textContent = toBengaliNumber(String(summary.unboughtItemsCount));
        document.getElementById('monthlyReportUnboughtItemsTotal').textContent = formatCurrency(summary.unboughtItemsTotal);
        zIndexCounter++;
        monthlySpendingModal.style.zIndex = zIndexCounter;
        monthlySpendingModal.classList.add('active'); 
        document.body.style.overflow = 'hidden';
    }
    ...
    function saveEditedCustomer() {
        ...
        if (!newName) {
            customAlert('গ্রাহকের নাম খালি রাখা যাবে না।');
            return;
        }
    ...
    function saveEditedTransaction() {
        ...
        if (gaveAmount > 0 && receivedAmount > 0) {
            customAlert('একই সাথে "দিলাম" এবং "পেলাম" এন্ট্রি করা যাবে না।'); return;
        }
        if (gaveAmount === 0 && receivedAmount === 0) {
            customAlert('অনুগ্রহ করে টাকার পরিমাণ লিখুন।'); return;
        }
